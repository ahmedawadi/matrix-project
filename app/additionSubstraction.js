    ...
    axios
      .post(
        process.env.backendDomainName +
          "/matrix/" +
          (additionOrSubstraction == 0 ? "add/" : "substract/"),
        dataToSend,
        { timeout: 12000 },
      )
      .then((res) => {
        window.open(
          `/${additionOrSubstraction == 0 ? "additionCalculation" : "substractionCalculation"}?matrixId=${res.data._id}`,
          "_blank",
        );
  ...
  return (
    <div className="xl:basis-[80%] bg-[#424143] py-[20px] xl:px-[50px] px-[15px]  flex flex-col">
      ...
      <div className="xl:pt-[80px] xl:text-[22px] pt-[30px] flex flex-col space-y-[30px] text-[#b5b5b5] text-[18px]">
        <div>{add_sousPageData.pageDesc}</div>
      ...
      <ReactModal
        ...
        overlayClassName={
          "xl:justify-center xl:items-center fixed top-0 left-0 right-0 bottom-0 flex bg-black bg-opacity-60 overflow-auto" +
          (matrixColumns > 7 ? "" : " xl:justify-center xl:items-center")
        }
