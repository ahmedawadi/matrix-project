...
export default function MatrixTranspose({ inputText, transposePageData }) {
  ...
  const calculate = () => {
    ...
    if (!matrix) {
      matrixAWarning.innerHTML = transposePageData.warnings.emptyCells;
      return;
    ...
    axios
      .post(process.env.backendDomainName + "/matrix/transpose/", dataToSend, {
        timeout: 12000,
      })
      .then((res) => {
      ...
      .catch((error) => {
        ...
        matrixAWarning.innerHTML = transposePageData.warnings.tryAgain;
      });
  };
      ...
      <div className="xl:pt-[80px] xl:text-[22px] pt-[30px] flex flex-col space-y-[30px] text-[#b5b5b5] text-[18px]">
        ...
        <div className="xl:px-[30px] xl:py-[0px] py-[5px] px-[10px] border-[1px] border-[#4a4a4a] shadow-[-1px_-1px_1px_rgba(0,0,0,0.7)] flex xl:flex-row flex-col space-y-[15px] xl:space-y-[0px] justify-between items-center w-full xl:h-[200px]">
          <div className="flex xl:flex-row flex-col xl:space-y-[0px] space-y-[5px] xl:space-x-[15px] text-[20px]">
            <div>{transposePageData.dimension1Desc}</div>
          ...
          <div className="h-full flex items-center">
            ...
            >
              {transposePageData.calculationButtonName}
            </button>
          </div>
        </div>
      </div>
