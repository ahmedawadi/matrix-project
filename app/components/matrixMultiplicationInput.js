...
export default function MatrixMultiplicationInput({
  openMatrixMultiplication,
  matrixLines,
  matrixColumns,
  matrixName,
  containsMatrixBand,
  sameBandOnTwoMatrices,
  oneMatrixWillBeUsed,
  dimensionsDesc,
}) {
    ...
    <div className="xl:px-[30px] xl:py-[20px] py-[5px] px-[10px] border-[1px] border-[#4a4a4a] shadow-[-1px_-1px_1px_rgba(0,0,0,0.7)] flex flex-col space-y-[20px] items-center justify-center w-full xl:min-h-[200px]">
      {containsMatrixBand ? (
        <div className="flex flex-col">
          {Array.from({ length: containsMatrixBand }).map((_, index) => (
            <div key={index} className="flex flex-col w-full">
              <div className="font-extrabold w-full">
                {sameBandOnTwoMatrices
                  ? "Ajouter la taille du bande des deux matrices"
                  : "* Ajouter la taille du bande de la matrice " +
                    (index == 0 ? "A" : "B") +
                    " :"}
                <input
                  type="number"
                  id={"matrix" + (index == 0 ? "A" : "B") + "Band"}
                  defaultValue={"1"}
                  className='w-[50px] h-[30px] mt-[5px] ml-[10px] p-[5px] text-[18px] font-medium text-black hover:bg-[url("../public/titleFont.png")] focus:bg-[url("../public/titleFont.png")]'
                  onChange={(event) => checkPositiveValue(event)}
                />
              </div>
              <div
                id={"matrix" + (index == 0 ? "A" : "B") + "BandWarning"}
      ...
      <div className="flex xl:flex-row flex-col space-y-[15px] xl:space-y-[0px] justify-between items-center w-full">
        <div className="flex flex-col space-y-[25px]">
          <div className="flex flex-col">
            <div className="flex xl:flex-row flex-col xl:space-y-[0px] space-y-[5px] xl:space-x-[15px] text-[20px]">
              <div>{dimensionsDesc.dim1Desc}</div>
              <div className="flex space-x-[15px]">
                ...
                <input
                  ...
                  defaultValue={"1"}
                  value={
                    matrixColumns && matrixName == "L"
                      ? matrixColumns
                      : matrixName == "R" && matrixLines
                        ? matrixLines
                        : undefined
                  }
                  className='w-[50px] h-[30px] p-[5px] text-[18px] text-black hover:bg-[url("../public/titleFont.png")] focus:bg-[url("../public/titleFont.png")]'
                  onChange={
                    !(
                      ...
                      ? (event) => changeMatrixBLines(event)
                      : null
                  }
                />
              </div>
          ...
          {oneMatrixWillBeUsed ? null : (
            <div className="flex flex-col">
              <div className="flex xl:flex-row flex-col xl:space-y-[0px] space-y-[5px] xl:space-x-[15px] text-[20px]">
                <div>{dimensionsDesc.dim2Desc}</div>
                <div className="flex space-x-[15px]">
                  <input
                    type="number"
                    id="matrixBLines"
                    defaultValue={"1"}
                    value={
                      matrixLines && matrixName == "R"
                        ? matrixLines
                        : matrixName == "L" && matrixColumns
                          ? matrixColumns
                          : undefined
                    }
                    className='w-[50px] h-[30px] p-[5px] text-[18px] text-black hover:bg-[url("../public/titleFont.png")] focus:bg-[url("../public/titleFont.png")]'
                    onChange={
                      !(matrixName == "R" || matrixName == "L")
                        ? (event) => changeMatrixAColumns(event)
                        : null
                    }
                  />
                  <div>X</div>
        ...
        <div className="h-full flex items-center">
          <button
            ...
            onClick={openMatrixMultiplication}
          >
            {dimensionsDesc.buttonName}
          </button>
        </div>
      </div>
    </div>
