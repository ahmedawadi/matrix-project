...
import {
  ...
  openmatricesTypeList,
} from "./multpilcationMatrices";
...
export default function SystemSolving({ systemResolutionPageData, inputText }) {
  ...
  const closeAlgorithmsTypesListInOutSideClick = useCallback((event) => {
    ...
    const algorithmsTypeListButton = document.getElementById(
      "algorithmsTypeListButton",
    );

    if (
      !(
        algorithmsTypeList?.contains(event.target) ||
        algorithmsTypeListButton?.contains(event.target)
      )
    )
  ...
  const getMatrix = () => {
    ...
    if (
      (matrixType == 1 && [1, 2, 3, 4, 5].includes(algorithm)) ||
      (algorithm == 0 && [2, 3].includes(matrixType))
    ) {
      ...
      if (matrixBand.value == "") {
        ...
        matrixBandWarning.innerHTML =
          "taille de la bande ne s'applique pas à la matrice";
        return;
      } else bandSize = matrixBand.value;

      if (matrixBandWarning.innerText != "") matrixBandWarning.innerHTML = "";
    } else if ([6, 7].includes(algorithm)) {
      //checking the addition of the number of iterations if we're working with iteratives methods
      const iterationsNumber = document.getElementById("iterationsNumber");
      const iterationsNumberWarning = document.getElementById(
        "iterationsNumberWarning",
      );
      ...
      if (iterativeMethod == 1) {
        if (iterationsNumber.value == "") {
          iterationsNumberWarning.innerHTML =
            "Nombre d'itérations ne peut pas être vide!";
          return;
        } else if (iterationsNumber.value == 0) {
          iterationsNumberWarning.innerHTML =
            "Nombre d'itérations ne peut pas être 0!";
          return;
        } else if (iterationsNumberWarning.innerText != "")
          iterationsNumberWarning.innerHTML = "";
      } else {
        //checking the addition of epsilon and the epsilon value (it can not be larger than 1)
  ...
  const calculate = () => {
    ...
    if (!systemData) {
      ...
      return;
    } else if (systemWarning.innerText != "") systemWarning.innerHTML = "";

    //system data is on that form [matrix, vector]
    if (!(systemData[0] && systemData[1])) return;
    ...
    if (algorithm != 0)
      dataToSend["algorithm"] = getAlgorithmNameOnTheServer(
        algorithm,
        matrixType,
      );
    ...
    if (
      ([1, 2, 3, 4, 5].includes(algorithm) && matrixType == 1) ||
      (algorithm == 0 && [2, 3].includes(matrixType))
    )
    ...
    if ([6, 7].includes(algorithm)) {
      if (iterativeMethod == 1)
        dataToSend["max_iteration"] =
          document.getElementById("iterationsNumber").value;
      else dataToSend["epsilon"] = document.getElementById("Epsilon").value;
    }
    axios
      .post(process.env.backendDomainName + "/matrix/solve/", dataToSend, {
        timeout: 12000,
      })
      .then((res) => {
        window.open(
          "/systemSolvingCalculation?matrixId=" + res.data._id,
          "_blank",
        );
      ...
      .catch((error) => {
        ...
        else systemWarning.innerHTML = "essayer une autre fois!";
  ...
  const chooseAlgorithm = (algorithm) => {
    ...
    setMatricesTypeList(
      algorithm == 0
        ? systemResolutionPageData.triangularSystemMatricesType
        : [1, 5].includes(algorithm)
          ? systemResolutionPageData.EGPP_cholesky_matricesType
          : [2, 3, 4].includes(algorithm)
            ? systemResolutionPageData.EG_LU__EGJ_matricesType
            : [],
    );
  };
        ...
        <div>
          ...
          <div className=" mt-[15px] w-full flex space-x-[10px]">
            <div className="w-[80%] font-bold flex flex-col space-y-[4px]">
              <div
                id="algorithmsTypeListButton"
                className="flex justify-between items-center border-2 px-[10px] py-[5px] cursor-pointer text-[16px] sm:text-[22px]"
                onClick={openAlgorithmsTypeList}
              >
                <div>{systemResolutionPageData.algorithms[algorithm]}</div>
                <FontAwesomeIcon
        ...
        {![6, 7].includes(algorithm) ? (
          <>
            ...
            <div className="w-[80%] font-bold flex flex-col space-y-[4px]">
              <div
                ...
                className="flex items-center justify-between border-2 px-[10px] py-[5px] cursor-pointer w-full sm:h-[45px] text-[16px] sm:text-[22px]"
                onClick={openmatricesTypeList}
              >
                <div>{matricesTypeList[matrixType]}</div>
                <div>
                  <FontAwesomeIcon
                    className="mt-[5px] w-fit"
                    size="lg"
              ...
              <ul
                ...
                className="bg-[#424143] text-[#b5b5b5] max-h-[200px] overflow-y-auto hidden border-y-2 border-x-2 font-bold w-full"
              >
                {matricesTypeList.map((matrixTypeName, index) => (
                  <li
                    key={index}
                    className={
                      "text-[20px] font-bold cursor-pointer hover:text-white hover:bg-[url('../public/titleFont.png')]" +
                      (index != matricesTypeList.length - 1
                        ? " border-b-[3px]"
                        : "")
                    }
                  >
          ...
          <>
            ...
            <ul className="list-disc sm:px-[45px] px-[15px]">
              {systemResolutionPageData.iterativeMethodsTypes.map(
                (iterativeMethodType, index) => (
                  <li key={index} className="w-full">
                    <label className="flex justify-between w-full font-semibold">
                      <div>{iterativeMethodType}</div>
                      <input
                        type="radio"
                        checked={iterativeMethod == index}
                        className="accent-black"
                        onClick={() => setIterativeMethod(index)}
                      />
                    </label>
                  </li>
                ),
            ...
            <div className="flex flex-col w-full">
              <div className="font-extrabold w-full">
                ...
                <input
                  ...
                  id="matrixBand"
                  defaultValue={"1"}
                  className='w-[50px] h-[30px] mt-[5px] ml-[15px] p-[5px] text-[18px] font-medium text-black hover:bg-[url("../public/titleFont.png")] focus:bg-[url("../public/titleFont.png")]'
                  onChange={(event) => checkPositiveValue(event)}
                />
              </div>
              <div
                id="matrixBandWarning"
                className="text-[#c92a1e] text-[18px]"
              ></div>
            </div>
          ) : [6, 7].includes(algorithm) && iterativeMethod == 1 ? (
            <div className="flex flex-col w-full">
              <div className="font-extrabold sm:text-[20px] text-[18px] flex-none w-full">
                ...
                <input
                  ...
                  id="iterationsNumber"
                  defaultValue={"1"}
                  className='w-[50px] h-[30px] mt-[5px] ml-[5px] p-[5px] text-[18px] font-medium text-black hover:bg-[url("../public/titleFont.png")] focus:bg-[url("../public/titleFont.png")] flex-1'
                  onChange={(event) => checkPositiveValue(event)}
                />
              </div>
              <div
                id="iterationsNumberWarning"
                className="text-[#c92a1e] text-[18px]"
              ></div>
            </div>
          ) : [6, 7].includes(algorithm) && iterativeMethod == 0 ? (
            <div className="flex flex-col w-full">
              <div className="font-extrabold sm:text-[20px] text-[18px] flex-none w-full">
                ...
                <input
                  ...
                  id="Epsilon"
                  defaultValue={"0.01"}
                  className='w-[70px] h-[30px] mt-[5px] ml-[5px] p-[5px] text-[18px] font-medium text-black hover:bg-[url("../public/titleFont.png")] focus:bg-[url("../public/titleFont.png")] flex-1'
                  onChange={(event) => checkPositiveValue(event)}
                />
              </div>
              <div
          ...
          <div className="flex xl:flex-row flex-col space-y-[15px] xl:space-y-[0px] justify-center space-x-[25px] items-center">
            <div className="text-[20px]">
              ...
              <input
                ...
                id="matrixSize"
                defaultValue={"1"}
                className='w-[50px] h-[30px]  ml-[10px] md:mt-0 mt-[8px] p-[5px] text-[18px] text-black hover:bg-[url("../public/titleFont.png")] focus:bg-[url("../public/titleFont.png")]'
...
function getSystemToSolveData(matrixName, vectorName, matrixSize) {
  ...
  for (let i = 0; i < matrixSize; i++) {
    ...
    for (let j = 0; j < matrixSize; j++) {
      ...
      else if (matrixElement.value == "") return null;
...
function getAlgorithmNameOnTheServer(algorithm, matrixType) {
  switch (algorithm) {
    ...
    case 1: {
      //gauss avec pivotage
      return matrixType == 0
        ? "pivot partiel gauss dense"
        : "pivot partiel gauss banded";
    }
    case 2: {
      //gauss ave elimination
      return matrixType == 0
        ? "gauss elimination symmetric dense matrix"
        : "gauss eliminatoion symmetric banded matrix";
    }
    case 3: {
      //gauss jordan
      return matrixType == 0
        ? "gauss jordan symmetric dense matrix"
        : "gauss jordan symmetric banded matrix";
    }
    case 4: {
      //decomposition LU
